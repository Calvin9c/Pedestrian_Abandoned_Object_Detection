...
# 修改plot_masks
def our_plot_masks(img, bin_masks, colors, alpha=0.5):
    """
    Args:
        img (tensor): img is in cuda, shape: [3, h, w], range: [0, 1]
        bin_masks (tensor): predicted masks on cuda, shape: [n, h, w]
        colors (List[List[Int]]): colors for predicted masks, [[r, g, b] * n]
    Return:
        ndarray: img after draw masks, shape: [h, w, 3]
    ...
    colors = colors[:, None, None, :]
    # [n, h, w, 1]0
    bin_masks = bin_masks[:, :, :, None]
    masks_color = bin_masks.repeat(1, 1, 1, 3) * colors * alpha
    inv_alph_masks = bin_masks * (-alpha) + 1
    masks_color_summand = masks_color[0]
    if num_masks > 1:
        inv_alph_cumul = inv_alph_masks[:(num_masks - 1)].cumprod(dim=0)
        masks_color_cumul = masks_color[1:] * inv_alph_cumul
    ...
    # 轉用numpy表示image
    return (img_gpu * 255).byte().cpu().numpy(), (masks_color_summand * 255).byte().cpu().numpy()
